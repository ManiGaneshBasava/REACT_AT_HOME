...
function Edit() {
    ...
    console.log(location.state.each)
    const details = location.state.each

    const dispatch = useDispatch()
    dispatch(authEdit(details))
    ...
    const submitHandlar = (e) => {
        e.preventDefault();
        const indexValue = state.findIndex((each) => { return each.email === details.email })
