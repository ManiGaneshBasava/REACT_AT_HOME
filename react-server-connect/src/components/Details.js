...
function Details() {
    ...
    const editCkeck = useSelector(state=>state.isEdit)
    ...
    const tableData = state.map((each ,key)=> {
        return (<tr key={key}>
            ...
            <td>{each.number}</td>
            <td>
             <Link to="/edit" style={{border:'2px solid black', height:'10px',width:'100%',padding:'5px', textDecoration:'none'}} state ={{each}}>Edit</Link> 
